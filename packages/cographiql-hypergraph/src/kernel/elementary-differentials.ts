...
export class ElementaryDifferentialsGenerator {
   ...
   */
  private static generateRecursive(n: number): RootedTree[] {
   ...
   */
  private static generatePartitions(n: number): number[][] {
    if (n === 0) return [[]];
    if (n === 1) return [[1]];
   ...
   */
  private static generateLabel(subtrees: RootedTree[]): string {
    if (subtrees.length === 0) return 'f';
   ...
   */
  static generateDomainSpecific(domain: string, order: number): RootedTree[] {
   ...
   */
  private static relabelForDomain(tree: RootedTree, domain: string): RootedTree {
    ...
    return {
      ...tree,
      label: tree.label.replace(/f/g, baseLabel),
      children: tree.children.map(child => this.relabelForDomain(child, domain)),
    };
  }
   ...
   */
  static symmetryFactor(tree: RootedTree): number {
